...
class masked_image_classifier():
    ...
    def _get_likelihood_targets(self, x_S, mask, X, Y):
        """
            ...
            - mask: the mask implied by subset of features S
            - X should be unnormalized, mask will be applied directly
        ...
        if self.device.type == 'cuda':
            ...
            for k in range(self.nclasses):
                ...
                # search returns return k nearest neighbors
                tau = D[:,-1].mean().item() # Mean distance to kNN's

                ### Actual Search
                lims, D, I = index.range_search(x_S.view(-1,W*H).cpu().numpy(), tau)
                # range_search returns all vectors with distance < radius = tau.
    ...
    def test(self, test_loader, epoch):
        ...
        try:
            ...
            print('Samples write failed, skipping')
